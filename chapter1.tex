...
Le mot \textit{tokamak} a deux étimologies débattues vennant du russe, soit тороидальная камера с магнитными magnetic катушками ("toroidalnaïa kamera s magnitnymi katouchkami' Chambre toroïdale avec bobines magnétiques), soit тороидальная камера с аксиальным магнитным полем ("toroidalnaïa kamera s aksial'nym magnitnym polem" chambre toroïdale avec un champ magnétique axial).

La machine a été inventé en union soviétique, les premiers développements ayant commencé dans les années 1950.
À cette époque la recherche sur le toroidal pinch configuration été fortement étudiés au Royaume-uni et aux États-Unis.
L'avantage du tokamak vient simplement de la stabilité augmenté grâce au champ magnétique toroîdal plus important.

Le développement réussi du tokamak est principalement du fai t de l'attention particulière à la réduction des impuretés et la séparation entre le plasma et la chambre à vide grâce à un limiteur.
Dans les années 1960, cela a mené à des plasma pure comparativement avec des températures d'électron d'environ \( \SI{1}{\kilo\electronvolt} \).
En 1970, ces résultats sont générallement acceptés et considérés comme significatifs.
...
Cette tache a été prises en charge et de nombreux tokamaks ont été construit dans plusieurs pays.

Il est rapidement devenu évident que le confinement de l’énergie est anormal.
...
Donc même si la théorie conditionnelle prédit que le temps de confinement collisionnel décroît comme \( 1/n \), où \( n \) est la densité d'électron, expérimentallement le temps de confinement augmente avec \( n \).

Tandis que les tokamaks plus grands ont été construit, les améliorations attendues du confinement ont été atteinte et des temps de confinement de \( \SI{100}{\milli\second} \) ont été obtenues dès 1980.
...
Le modèle le plus simple consistent avec les résultats expérimentaux donnait \( \tau_E \approx n a^2 \), où \( a \) est le petit rayon du plasma, et une extrapolation des données expérimentales indiquait qu'un petit rayon \( \sim \SI{2}{meter} \) pourrait être suffisant pour un réacteur.

Ce pré-rtequis sur la taille du plasma semblait acceptable et le design de grands tokamaks expérimentaux ont été fait dans plusieurs pays.
...
Des atomes neutres à haute vitesse sont injecté dans le plasma et donnent leur énergie par des collisions.
Une autre méthode de chauffage est de lancer des ondes à fréquences radio (RF) dans le plasma de façon à ce qu'elles soient absorbé à des surfaces de résonnance à l'intérieur du plasma.
Plusieurs fréquences ont été utilisé mais la plus prometteuse était le chauffage par résonnance cyclotron des ions.
L'injection de neutres et le chauffage RF ont atteinent des températures de plusieurs \( \SI{}{\kilo\electronvolt} \) au début des années 1980.

Les plus hautes températures produitent par les chauffages additionnels ont permis de clarifié la dépendance du temps de confinement à la température.
Une prédiction théorique basé sur le transport d'énergie par de simples collisions donnerait un confinement qui se dégraderait avec l'augmentation de la densité mais s'améliorerait avec une température plus grande.
Malheureusement, dans ces plasmas plus chaud, le temps de confinment a été trouvé de décroire avec la densité et la température.
Lorsque la puissance de chauffage a été applied au plasma, le temps de confinement a chuté.
Ces résultats étaient décevant mais n'ont pas présenté un obstacle fondamental.
En utilisant des résultats empiriques donnés par une variété d'expériences, le temps de confinement a été prédit de s'amélioré avec la taille du tokamak et son courant du plasma plus grand.

Une heureuse découverte faite par le tokamak ASDEX était l'existence de mode d'opération du plasma qui ont un plus long temps de confinement que l'opération normale.
Ce mode est appelé H-mode.
La transition à ce mode confinement amélioré a été trouvé dans un plasma avec un divertor et à un niveau de chauffage du plasma suffisant.
La transition apparait brusquement et semble être associé à un confinement amélioré au bord du plasma.
