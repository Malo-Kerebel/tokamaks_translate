...
\section{Déplacement d'une particule selon \( \textbf{B} \)}
...
Une accélération de la particule selon le champ magnétique est introduite s'il y a un champ électrique parallèle à \( \textbf{B} \) ou un gradient de \( \textbf{B} \) pzrallèle à \( \textbf{B} \).

\subsubsection*{Accélérayion par \( \textbf{E}_\parallel \)}

Un champ électrique parallèle donne simplement une accélération donnée par

\[
	\frac{\dd}{\dd t} \left( m_j v_\parallel \right) = e_j \mathbf{E_\parallel} 
\]

si \( \mathbf{E_\parallel} \) est une fonction de \( t \), \( v_\parallel \) est donné par
...
Si \( \mathbf{E_\parallel} \) est une fonction de la distance \( x_\parallel \) selon la magnitude du champ et du temps, il faut alors résoudre l'équation

\[
	\frac{\dd}{\dd t} \left( m_j \frac{\dd x_\parallel}{\dd t} \right) = e_j \mathbf{E_\parallel} \left( x_\parallel, t \right).
\]

\subsubsection*{Accélérayion par \( \nabla_\parallel \mathbf{B} \)}

Une particule chargée se déplaçant parallèlement à un champ magnétique n'est soumise à aucune force magnétique.
Cependant, si la particule a aussi une vitesse perpendiculaire au champ magnétique et lz champ magnétique a un gradient parallèle à \( \mathbf{B} \), il y a une force parallèle au champ magnétique sur la particule au centre de la gyro-orbite.

\begin{figure}
\centering
    % Minipage pour la légende (à gauche)
    \begin{minipage}[c]{0.35\textwidth}
        \caption{Le gradient de \( \mathbf{B} \) parallèle ) \( \mathbf{B} \) fait apparaitre une composante de la force \( e_j \left( \mathbf{v} \times \mathbf{B} \right) \) selon la direction du déplacement du centre guide.}
        \label{fig:gradB}
    \end{minipage}\hfill
...
La force sur la particule est \( e_j \left( \mathbf{v} \times \mathbf{B} \right) \).
...
\begin{equation}
	F = \alpha\left\vert e_j \left( \mathbf{v} \times \mathbf{B} \right) \right\vert
...
\( \rho \) étant le rayon de Larmor. Puisque \( \nabla \cdot \mathbf{B} \) et \( B_r = r \partial B_r / \partial r \),
